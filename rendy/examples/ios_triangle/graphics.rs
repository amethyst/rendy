...
impl Graphics {
    pub fn new(window: &Window) -> Self {
        ...
        graph_builder.add_node(
            RenderPipeline::builder().into_subpass().with_color_surface().into_pass().with_surface(
                surface,
                Some(ClearValue { color: ClearColor { float32: [0.392, 0.584, 0.929, 1.0] } }),
            ),
        );

        let graph = Some(graph_builder.build(&mut factory, &mut families, &user_data).unwrap());

        Graphics { factory, families, user_data, graph }
    }
...
impl SimpleGraphicsPipelineDesc<Backend, CustomUserData> for RenderPipelineDesc {
    ...
    fn load_shader_set(
        ...
        builder.build(factory, SpecConstantSet::default()).expect("Unable to build source shaders")
    }
