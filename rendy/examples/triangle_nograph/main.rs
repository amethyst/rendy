...
fn run2<B: Backend>(
    ...
    unsafe {
        ...
        //framebuffer_width = target.extent().width;
        //framebuffer_height = target.extent().height;
        //framebuffer_layers = target.backbuffer()[0].layers();

        //views = target.backbuffer().iter().map(|image| unsafe {
        //    factory
        //        .device()
        //        .create_image_view(
        //            image.raw(),
    ...
    //let mut framebuffers = views.iter().map(|view| unsafe {
    //    let fb = factory
    //        .device()
    //        .create_framebuffer(
    //            &render_pass,
