    ...
    B: Backend,
{
    ...
    pub fn with_queue_type(self) -> CommandBuffer<B, hal::queue::QueueType, S, L, R>
    where
        C: Capability,
    {
        CommandBuffer {
...
pub unsafe trait BeginInfo<'a, B: Backend, L> {
    ...
    fn inheritance_info(self) -> hal::command::CommandBufferInheritanceInfo<'a, B>;
}
...
unsafe impl<'a, B> BeginInfo<'a, B, SecondaryLevel> for hal::pass::Subpass<'a, B>
where
    B: Backend,
{
...
unsafe impl<'a, B, F> BeginInfo<'a, B, SecondaryLevel>
    for (hal::pass::Subpass<'a, B>, Option<&'a F>)
where
    B: Backend,
...
unsafe impl<'a, B, F> BeginInfo<'a, B, SecondaryLevel> for (hal::pass::Subpass<'a, B>, &'a F)
where
    B: Backend,
    F: std::borrow::Borrow<B::Framebuffer>,
{
    ...
    B: Backend,
{
        ...
        P: RenderPassRelation<L>,
    {
        ...
        unsafe {
            hal::command::CommandBuffer::begin(
                self.raw(),
    ...
    B: Backend,
{
    ...
    pub fn finish(mut self) -> CommandBuffer<B, C, ExecutableState<U, P>, L, R> {
        unsafe {
            hal::command::CommandBuffer::finish(self.raw());
    ...
    B: Backend,
{
    ...
    /// [waiting]: ..hal/device/trait.Device.html#method.wait_for_fences
    /// [`Fence`]: ..hal/trait.Backend.html#associatedtype.Fence
    /// [submitted]: ..hal/queue/struct.CommandQueue.html#method.submit
