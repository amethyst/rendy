...
/// Recording commands inside or outside renderpass.
    ...
    B: gfx_hal::Backend,
{
    ...
    ) -> RenderPassEncoder<'_, B>
    where
        C: Supports<Graphics>,
    {
        let graphics = self.capability.supports().unwrap();
	...
	pub fn execute_commands(&mut self, submittables: impl IntoIterator<Item = impl Submittable<B>>) {
