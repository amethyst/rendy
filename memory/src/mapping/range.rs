...
pub(crate) fn mapped_fitting_range(ptr: NonNull<u8>, range: Range<u64>, fitting: Range<u64>) -> Option<NonNull<u8>> {
...
pub(crate) fn mapped_sub_range(ptr: NonNull<u8>, range: Range<u64>, sub: Range<u64>) -> Option<(NonNull<u8>, Range<u64>)> {
...
pub(crate) unsafe fn mapped_slice_mut<'a, U>(ptr: NonNull<u8>, range: Range<u64>) -> Result<&'a mut [U], MappingError> {
...
/// User must ensure that:
/// * returned slice doesn't outlive mapping.
pub(crate) unsafe fn mapped_slice<'a, U>(ptr: NonNull<u8>, range: Range<u64>) -> Result<&'a [U], MappingError> {
    let size = (range.end - range.start) as usize;
    assert_eq!(size % size_of::<U>(), 0, "Range length must be multiple of element size");
    let offset = ptr.as_ptr() as usize;
    if offset % align_of::<U>() > 0 {
