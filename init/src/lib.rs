...
impl<B: Backend> Rendy<B> {
    pub fn init(
        ...
        let (factory, families) = rendy_factory::init_with_instance(Instance::new(instance), config)?;
...
pub fn pick_backend(priority: impl IntoIterator<Item = rendy_core::Backend>) -> Option<EnabledBackend> {
    priority.into_iter().filter_map(|b| std::convert::TryInto::try_into(b).ok()).next()
}
